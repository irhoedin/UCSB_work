...
class KE2400(GPIB_SetUp):
    ...
    gpib_adr = "GPIB0::13::INSTR"

    def __init__(self, gpib_adr):
    ...
    def iv_sweep_core(self, v_start, v_end, v_step,
        ...
        inpt = self.inst.write(':SENS:CURR:DC:NPLC ' + str(nplc))
...
class KE2400withLED(KE2400):
    """
    Conductivity measurements with KE2400 and THORLABS DC2200 LED controller
    """

    def __init__(self, gpib_addr='GPIB0::13::INSTR'):
        rm = visa.ResourceManager(VISA_PATH)
        ...
        for res in res_list:
            if 'USB0' in res:
                self.DC2200 = rm.open_resource(res)
                try:
                    out = self.DC2200.query('*IDN?')
                    if 'DC2200' in out:
                        DC2200_found = True
                        print 'DC2200 is confirmed\n{}'.format(res)
                        break
                except:
                    print 'except'
                    pass
        ...
        if not DC2200_found:
            print 'Cound not find DC2200. Check the connection'
...
class Conductivity(GPIB_SetUp):
    ...
    def __init__(self, KE6220_adrs, KE2400_adrs):
        """gpib_add1 for KE6220, gpib_add2 for KE2400"""

        GPIB_SetUp.__init__(self, KE6220_adrs, KE2400_adrs)
    ...
    def measure(self, path, filename, curr_start=-1e-3, curr_stop=1e-3, curr_step=1e-4,
                sour_del=.01, nplc=1, graph=True, bias_curr=0., c_cmpl=100., v_cmpl=50.,
        ...
        inpt = self.K6220.write('OUTP:ISH OLOW')  # triaxial inner shield is output low
        inpt = self.K6220.write('OUTP:LTE ON')  # Output low is internally connected to Earth Ground
...
class TFT(GPIB_SetUp):
    """TFT measurement using two sourcematers
    
    SMU1: K2400 for source-gate
    SMU2: K6487 for source-drain
    """

    SMU1ADRS = 'GPIB0::13::INSTR' #gate bias
    SMU2ADRS = 'GPIB0::11::INSTR'  #

    def __init__(self, smu1_add=SMU1ADRS, smu2_add=SMU2ADRS):

        rm = visa.ResourceManager(VISA_PATH)
        self.smu1 = rm.open_resource(smu1_add)
        self.smu2 = rm.open_resource(smu2_add)

        self.smu1_info = self.smu1.query('*IDN?')
        self.smu2_info = self.smu2.query('*IDN?')

        self.confirm_inst(self.smu1_info, '2400')
        self.confirm_inst(self.smu2_info, '6487')
    ...
    def meas_output(self, path, filename, v_start=0., v_end=100., v_step=5.,
        ...
        # setup smu1 (K2400)
        self.smu1.timeout = int(2000)


        inpt = self.smu1.write('*RST')
        inpt = self.smu1.write(':SOUR:FUNC VOLT')
        inpt = self.smu1.write(':SOUR:VOLT:MODE FIXED')
        inpt = self.smu1.write(':SOUR:VOLT:RANG 120')
        inpt = self.smu1.write(':SOUR:VOLT:LEV 0')
        inpt = self.smu1.write(':SENS:CURR:PROT ' + str(cmpl))
        inpt = self.smu1.write(':SENS:FUNC "CURR"')
        inpt = self.smu1.write(':SENS:CURR:RANG 0.01')
        inpt = self.smu1.write(':FORM:ELEM CURR')

        # setup smu2 (K6487)
        inpt = self.smu2.write('*RST')
        inpt = self.smu2.write('SOUR:VOLT:RANG 100')
        inpt = self.smu2.write('SOUR:VOLT:ILIM 0.001')
        inpt = self.smu2.write('SENS:CURR:NPLC ' + str(nplc))
        inpt = self.smu2.write('SOUR:VOLT 0')
        inpt = self.smu2.write('SYST:ZCH OFF')

        # turn on the voltage
        inpt = self.smu1.write(':OUTP ON')
        inpt = self.smu2.write('SOUR:VOLT:STAT ON')
        ...
        for i, v_g in enumerate(v_g_list):
            inpt = self.smu1.write(':SOUR:VOLT:LEV ' + str(v_g))
            full_isd_list.append([])
            full_igd_list.append([])
            full_vsd_list.append([])

            for v_sd in v_sd_list:
                inpt = self.smu2.write('SOUR:VOLT ' + str(v_sd))
                time.sleep(waiting_time)
                outp2 = self.smu2.query('READ?')
                i_sd = float(outp2.split(',')[0][:-1])

                outp1 = self.smu1.query('READ?')
                i_vd = float(outp1)
        ...
        inpt = self.smu1.write('OUTP OFF')
        inpt = self.smu2.write(':SOUR:VOLT:STAT OFF')

        df = pd.DataFrame(full_i_list).T
        df.columns = ['Vg'+str(int(v_g)) for v_g in v_g_list]
        df['Vsd'] = full_v_lsit[0]

        df.to_csv(path_file + '.csv')

        return df

    def meas_transfer(self, path, filename, v_start=-10., v_end=100.,
                      v_step=5., v_sd_list=[20.,50.,80.],
                      reverse=True, cmpl=0.1, nplc=6, wait=0.01):
        ...
        :param cmpl: float, compliance=0.1 [A]
        ...
        for i, v_sd in enumerate(v_sd_list):
            ...
            for v_g in v_g_list:
                inpt = self.smu1.write('SOUR:VOLT:LEV ' + str(v_g))
                time.sleep(wait)
                outp_i_sd = self.smu2.query('READ?')
                outp_i_g = self.smu1.query('READ?')
                ...
                for volt, curr_sd in zip(full_v_list, full_isd_list):
                    ax.plot(volt, curr_sd, color='red')

                for volt, curr_g in zip(full_v_list, full_ig_list):
                    ax.plot(volt, curr_g, color='blue')
...
class Picoammeter(GPIB_SetUp):
    ...
    def __init__(self, address=PAM_ADRS):
        rm = visa.ResourceManager(VISA_PATH)
        self.my_instrument = rm.open_resource(address)

        self.inst_info = self.my_instrument.query('*IDN?')

        self.confirm_inst(self.inst_info, '6487')
    ...
    def meas_curr_sweep(self, path, filename, v_start, v_end, v_step,
        ...
        self.my_instrument.timeout = int(nplc * 60 * 1000 * 2)

        inpt = self.my_instrument.write('*RST')
        inpt = self.my_instrument.write('SOUR:VOLT:RANG ' + str(rang))
        inpt = self.my_instrument.write('SOUR:VOLT:ILIM ' + str(ilim))
        inpt = self.my_instrument.write('SENS:CURR:NPLC ' + str(nplc))

        volt_list = np.arange(v_start, v_end + v_step, v_step)
        current_list_raw = []

        for volt in volt_list:
            inpt = self.my_instrument.write('SOUR:VOLT ' + str(volt))
            inpt = self.my_instrument.write('SOUR:VOLT:STAT ON')
            inpt = self.my_instrument.write('SYST:ZCH OFF')
            outp = self.my_instrument.query('READ?')
            ...
            inpt = self.my_instrument.write('SOUR:VOLT:STAT OFF')
...
class KE2602(GPIB_SetUp):
    ...
    def __init__(self, gpib_addr=cnsi_addr):
        GPIB_SetUp.__init__(self, gpib_addr)
        self.confirm_inst(self.info, "2602")
    ...
    def iv_sweep_core(self, v_start, v_end, step_num, bottom_anode=False,
        ...
        self.inst.timeout = int(sweep_time * 1000 * 2)  # double of the sweep time, to be safe
        ...
        inpt = self.inst.write('SweepVLinMeasureI(smua, %s, %s, %s, %s)' \
                               % (str(v_start), str(v_end), str(sdel), str(step_num)))

        output = self.inst.query('printbuffer(1, %s, smua.nvbuffer1.readings)' % str(step_num))
    ...
    def meas_tpc(self, filename, area=0.0432, bias=0.0, waittime=.1, nplc=1.,
        ...
        while True:
            ...
            inpt = self.inst.write('smua.measure.i(smua.nvbuffer1)')
            current_now = self.inst.query(
    ...
    def meas_tpv(self, start_bias=0.0, nplc=.1, cmpl=.1):
        ...
        while True:
            ...
            try:
                ...
                if current_now < 0:
                    bais_now += .01
                    inpt = self.inst.write('smua.source.levelv = ' \
                                           + str(bias_now))
                    print now, bias_now
    ...
    def meas_light_depen(self, v_start=1.1, delay=0.5, v_step=0.01, nplc=1.,
        ...
        inpt = self.inst.write('smua.measure.nplc = ' + str(nplc))
        inpt = self.inst.write('smua.source.limiti = ' + str(cmpl))
        inpt = self.inst.write('smua.source.levelv = 0')
        inpt = self.inst.write('smua.source.output = smua.OUTPUT_ON')
        ...
        while point_beyond_voc:
            ...
            inpt = self.inst.write('smua.source.levelv = '  + str(voltage))
            time.sleep(delay)

            inpt = self.inst.write('smua.measure.i(smua.nvbuffer1)')
            current_s = self.inst.query(
                'printbuffer(1, 1, smua.nvbuffer1.readings)')
        ...
        inpt = self.inst.write('smua.measure.i(smua.nvbuffer1)')
        current_s = self.inst.query('printbuffer(1, 1, smua.nvbuffer1.readings)')
        current = float(current_s)
    ...
    def mppt(self, filename, watetime=0., v_step=0.005, start_v=1.0, area=0.0432,
        ...
        self.inst.write('smua.reset()')
        self.inst.write('smua.measure.nplc = ' + str(nplc))
        self.inst.write('smua.source.limiti = ' + str(cmpl))
        self.inst.write('smua.source.levelv = 0.0')
        ...
        while True:
            ...
            self.inst.write('smua.source.levelv = ' + str(voltage))
            self.inst.write('smua.measure.i(smua.nvbuffer1)')
            current_read = self.inst.query(
                'printbuffer(1, 1, smua.nvbuffer1.readings)')
            ...
            self.inst.write('smua.source.levelv = ' + str(voltage_2))
            self.inst.write('smua.nvbuffer1.clear()')
            self.inst.write('smua.measure.i(smua.nvbuffer1)')
            current_read2 = self.inst.query(
                'printbuffer(1, 1, smua.nvbuffer1.readings)'
            )
